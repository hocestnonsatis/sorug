...
//! `build.rs` from `data/idna_ranges.txt`). Value-producing remaps stay in
//! [`map_char`].
//!
//! WHATWG domain parser (`beStrict = false`):
//! - ASCII input → lowercase pass-through (even if ACE labels are invalid)
//! - Non-ASCII → map (if needed) → split labels → Punycode-encode non-ASCII labels
...
fn char_needs_uts46_map(c: char) -> bool {
    ...
    // ß kept (non-transitional); other non-ASCII may need NFKC and/or case fold.
    if c == 'ß' {
        return false;
    }
    let nfd: String = c.nfd().collect();
...
fn encode_punycode(input: &str, out: &mut impl AppendBuf) -> Result<(), ()> {
    ...
    for &cp in chars {
        if cp < 0x80 {
            out.push(char::from(cp as u8).to_ascii_lowercase());
            basic_count += 1;
        }
    }
